...
def convert(input_path, target_format):
    ...
    """
    target_format = target_format.lower().strip('.')  #Normalize the format
    
    base = os.path.splitext(input_path)[0]
    output_path = f"{base}_converted.{target_format}"

    if target_format == 'gif':
        # Better quality GIF using palette optimization since gif quality is poor if done directly
        # 2 steps in gif for palette generation to improving color quality
        palette_path = f"{base}_palette.png"
        subprocess.run([
            "ffmpeg", "-y", "-i", input_path,
            "-vf", "fps=10,scale=320:-1:flags=lanczos,palettegen",  # creates color palette 
            palette_path
        ...
        subprocess.run([
            ...
            output_path
        ], check=True, capture_output=True)   # uses palette to improve GIF quality
...
def can_handle(input_path, target_format):  # checks if the converter supports the givem formats
